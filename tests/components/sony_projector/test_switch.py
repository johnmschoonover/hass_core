"""Tests for the Sony Projector switch entity."""
...
async def test_async_setup_entry_adds_entity(hass: HomeAssistant) -> None:
    ...
    with (
        ...
        patch.object(hass, "async_add_executor_job", executor_mock),
    ):
...
async def test_switch_async_update_sets_state(hass: HomeAssistant) -> None:
    ...
    projector = MagicMock()
    entity = SonyProjectorSwitch(
        hass, projector=projector, name="Projector", host=TEST_HOST
    )
